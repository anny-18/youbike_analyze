...
及「[YouBike2.0臺北市公共自行車即時資訊](https://data.gov.tw/dataset/137993)」。另外，因資料清理所需，也同樣使用「[新北市公共自行車租賃系統(YouBike2.0)](https://data.ntpc.gov.tw/datasets/010e5b15-3823-4b20-b401-b1cf000550c5)」，「[桃園公共自行車即時服務資料](https://opendata.tycg.gov.tw/datalist/5ca2bfc7-9ace-4719-88ae-4034b9a5a55c)」。  
...
### 3. 資料清理：修正或標記異常值
在此階段，將確認各欄位是否有缺失或異常值。並針對具體情況採取兩種對策：  
1. 修改回正確值  
2. 標記並於分析時排除  
...
新增欄位is_excluded，預設其值為 0。後續若發現異常資料，將會於該欄位標記為 1，代表要在分析階段排除。
...
#檢查start_staion
filter(all_data, is.na(start_station)|start_station=="") #1筆為NA(NULL)
#檢查end_staion
filter(all_data, is.na(end_station)|end_station=="") #10筆為NA(NULL)
#檢查duration
filter(all_data, is.na(duration)) 
#檢查ride_date
filter(all_data, is.na(ride_date))
```
- 缺少「借車站點」與「還車站點」的資料共17筆（原檔共計 35 筆）。由於占比極小。且無法推斷原始站點，故將其標記為異常並排除於後續分析中。
...
因此：
  - start_station合理站點數：1622站以下。
  - end_station合理站點數：由於可包含其他縣市的所有站點，應約3083站或以下。
```{r check station,eval=TRUE, echo=TRUE}
#檢查站點個數是否合理(start_station)
length(unique(all_data$start_station)) 
#檢查站點個數是否合理(end_station)
...
```
找出 start_staion 欄位中未出現在 station_data 中的站點名稱。
...
最後確認start_station名稱皆已完全修正 ( 所有站名皆在station_data名單內 ) 。
...
not_in_station_data_distinct
```
 <u><strong>修正 end_station 中的異常站名：</strong></u><br>
end_station 資料中可能包含來自其他縣市的站點名稱，例如桃園。由於需修正清單內大部分站點位於台北市及新北市之外，少部分為錯字，因此：
1. 先將桃園站點資料標記排除
2. 檢視所有異常站名後，建立正確的對應站名清單一次修正。
...
not_in_station_data_distinct
```
人工檢視異常站名後建立對應正確站名清單 corrections。  
內容如下：  

- before：錯誤站名。  
- after：對應修正後的站名，若為 "1" 表示將該資料排除。  
  
※ cat() 原為除錯時觀察用，此處已註解停用  
...
```
修正end_station的站名。
```{r correct end_station2, eval=TRUE, echo=TRUE}
#修正站名
...
not_in_station_data_distinct
```
部分站點（如福壽公園、後港公園等）在臺北與新北擁有同名站點，且原始資料無法進一步辨識其所屬縣市，為避免地區分析誤判，故排除此類站點之資料（共 75,229 筆，占總資料約 0.79%）。
...
```
### 4. 資料分析
在資料分析階段，首先為資料新增平假日分類、時間段、每日平均借還差值等欄位，接著根據每日平均差值將站點分為兩類進行分析，並擬定調度策略。  
...
```
根據租借紀錄，分別建立借出表 ( borrow_df ) 及歸還表 ( return_df ) ，
...
代表部分站點的車輛會隨時間自然流動回原站，不需特別安排大量調度作業，僅需掌握時間差即可達成平衡。
...
```
#### 整日不平衡站點之調度規劃
針對每日借還差值超過 10 輛的站點，計畫以「每日平均借還差值」為基準進行調度，  
並為避免無車可借或無空位可還的情況：  
...
輕度調度清單：  
對於屬於「短時間不平衡」的站點，依據不同時間段（平日早上、晚上，假日午間與夜間）列出需要輕度調整的站點名單。

調度配對清單：  
針對「整日不平衡」站點，依據供需配對結果建立調度建議，包括需補車與可提供車輛的站點配對與數量建議。
...
本次分析以Youbike   2.0之租借紀錄為資料來源，聚焦於台北市站點的供需情況。藉此觀察租借模式並找出不平衡站點提出調度建議。  
  
**2. 清理與處理步驟  **  
  
針對資料中錯誤或模糊的站點名稱進行人工修正，並排除異常使用紀錄（如借用時間小於1小時或大於6小時），將租借時間分成平假日與時間段分類，以利觀察站點間的流動差異。  
  
**3. 重要發現**
  
- 台北市多數站點呈現不同時段的供需互補現象：  
觀察結果顯示，多數站點在下列時段間存在顯著的供需互補現象，顯示車輛可於不同時段間自然流動回補，具備一定程度的自我平衡能力：  
  - 平日早上與晚上通勤時段（CommuteAM ⇄ CommutePM）  
  - 平日白天與夜間（DayTime ⇄ NightTime）  
  - 假日早上與夜間（Morning ⇄ Night）  
- 等待車輛回流期間，仍可能出現供需落差，需制定調度策略：  
儘管整體上具備回補能力，但在等待車輛回流的期間仍有車輛不足或無位可還的風險。因此仍須制定適當調度計畫，以確保使用者體驗。

從圖中可見，方框內的站點在不同時段呈現借出與歸還的反向變化，表示若能容忍短暫等待，車輛有機會自然回流，無須立即調度。  
<div style="border: 1px solid black; display: inline-block;">
  <img src="平日氣泡圖_標記.png" width="100%">
</div>
<div style="border: 1px solid black; display: inline-block;">
  <img src="假日氣泡圖_標記.png" width="100%">
</div>
...
1. 納入車輛流動方向（站點間流向）  
　本次分析僅統計各站點的借出與歸還數量，尚未追蹤自行車實際的移動方向與流向。  
　若能進一步結合「出發站點 ➝ 到達站點」的資料，可更精準掌握車輛流動路徑，協助優化調度策略與路線規劃。  
2. 更準確之資料紀錄  
原始資料中的借車與還車時間僅記錄至小時級（hour），缺乏分鐘資訊。  
儘管可透過騎乘時長計算得知使用區間，但若能取得記錄至「分鐘」的時間資料，將能進行更精細的流量與高峰期分析。  
  
3. 加入使用者識別（如會員ID）以分析使用行為  
若資料中包含使用者 ID，可追蹤使用者行為模式，如：  
- 租借頻率與週期  
- 常用路線與站點偏好  
- 騎乘距離與時間分佈  
便可進行使用者分群，進而針對不同族群制定更精準的行銷與服務推廣策略。

### 7. 附錄（程式碼與資料摘要）  
##### 7.1. 主要資料表說明
```{r all_data_column, eval=TRUE, echo=FALSE}
df <- tibble(
  欄位名稱 = c("start_time", "start_station", "end_time", "end_station", "duration", "ride_date", "is_excluded", "weekday_type", "time_period"),
  資料型態 = c("日期時間", "字串", "日期時間", "字串", "數值", "日期", "整數", "字串", "字串"),
  說明 = c("借車時間", "借車站點", "還車時間", "還車站點", "借用時長（秒）", "借用日期", "異常資料標記（1為異常排除，0為正常）", "平假日分類", "時間段分類")
)
kable(df, format = "html", caption = "all_data") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover", "condensed", "bordered"))
```
```{r top_100_staiton_data, eval=TRUE, echo=FALSE}
df <- tibble(
  欄位名稱 = c("station", "weekday_type", "time_period",  "capacity", 
            "borrow_count", "return_count", "borrow_per_day", "return_per_day", "avg_diff_per_day",
            "district", "latitude", "longitude"),
  資料型態 = c("字串", "字串", "字串", "數值", "整數", 
            "整數", "整數", "數值", "數值", 
            "字串", "數值", "數值"),
  說明 = c("站點名稱", "平假日分類", "時間段分類", "站點容量", 
         "總借車次數", "總還車次數", "每日平均借車次數", "每日平均還車次數",  "每日平均借還差值",
         "行政區", "緯度", "經度")
)
kable(df, format = "html", caption = "top_100_staiton_data") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover", "condensed", "bordered"))
```
##### 7.2. 分析環境說明  

- 分析工具：RStudio  
- 語言版本：R version 4.4.3 (2025-02-28 ucrt) — Trophy Case  
- 使用套件：  
  - tidyverse（資料清理與視覺化）  
  - stringdist（字串相似度比對）  
  - kableExtra（美化表格）  
